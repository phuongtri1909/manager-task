                ...
                <div class="filter-group">
                    ...
                    <div class="filter-item">
                        ...
                        <select id="status_filter" name="status" class="filter-input">
                            ...
                            <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>Chưa thực hiện</option>
            ...
            @if (request('title') || request('status') || request('overdue'))
                <div class="active-filters">
                    ...
                    @if (request('status'))
                        <span class="filter-tag">
                            <span>Trạng thái: 
                                {{ request('status') == 'pending' ? 'Chưa thực hiện' : 
                    ...
                    @if (request('overdue') !== null && request('overdue') !== '')
                        <span class="filter-tag">
                            ...
                            <a href="{{ request()->url() }}?{{ http_build_query(request()->except('overdue')) }}" class="remove-filter">×</a>
                        </span>
                    @endif
                </div>
            @endif
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @elseif($task->deadline->diffInDays(now()) <= 3)
                                            <span class="filter-tag" style="background-color: #fff8e1; color: #ff8f00;">Sắp hết hạn</span>
                                        @endif
                                    </td>
                                    <td class="text-center">
                                        <span class="status-badge {{ 
                                            $task->status === 'pending' ? 'status-inactive' : 
                                            ($task->status === 'in_progress' ? 'bg-info text-white' : 
                                            ($task->status === 'completed' ? 'status-active' : 'bg-info text-white')) 
                                        }}">
                                            {{ 
                                    ...
                                    <td>
                                        <div class="action-buttons-wrapper">
                                            <a href="{{ route('tasks.show', $task) }}" class="action-icon view-icon" title="Xem chi tiết">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            @if(Auth::user()->id === $task->created_by || Auth::user()->isAdmin())
                                                <a href="{{ route('tasks.edit', $task) }}" class="action-icon edit-icon" title="Chỉnh sửa">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route('tasks.destroy', $task) }}" method="POST" class="delete-action-container d-inline">
                                                    @csrf
                                                    @method('DELETE')
