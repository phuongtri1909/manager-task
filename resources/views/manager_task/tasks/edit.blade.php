                    ...
                    <div class="form-tabs">
                        ...
                        <div class="form-group">
                            ...
                            <input type="datetime-local" class="custom-input {{ $errors->has('deadline') ? 'input-error' : '' }}" 
                                id="deadline" name="deadline" value="{{ old('deadline', $task->deadline->format('Y-m-d\TH:i')) }}" required>
                        ...
                        <div class="form-check-group">
                            <div class="custom-checkbox">
                                <input type="checkbox" id="for_departments" name="for_departments" value="1" 
                                    {{ old('for_departments', $task->for_departments) ? 'checked' : '' }} onchange="toggleTaskType()">
                        ...
                        <div id="department_section" class="{{ old('for_departments', $task->for_departments) ? '' : 'd-none' }}">
                            <div class="form-group">
                                ...
                                <select class="custom-input select2 {{ $errors->has('departments') ? 'input-error' : '' }}" 
                                    ...
                                    @foreach($departments as $department)
                                        <option value="{{ $department->id }}" {{ (old('departments', $selectedDepartments) && in_array($department->id, old('departments', $selectedDepartments))) ? 'selected' : '' }}>
                                            {{ $department->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div id="user_section" class="{{ old('for_departments', $task->for_departments) ? 'd-none' : '' }}">
                            <div class="form-group">
                                ...
                                <select class="custom-input select2 {{ $errors->has('users') ? 'input-error' : '' }}" 
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ (old('users', $selectedUsers) && in_array($user->id, old('users', $selectedUsers))) ? 'selected' : '' }}>
